...
void samplePoints(std::vector<zVector>& trainingSamples, std::vector<float>& sdfGT, vector<zVector>& polygon)
{
    ...
    for (float x = -50; x <= 50; x += 3.0f)
    {
        for (float y = -50; y <= 50; y += 3.0f)
        {
            zVector pt(x, y, 0);
...
float blendOrientedBoxSDFs(zVector pt, std::vector<zVector>& centers, std::vector<float>& angles, float width = 8.0f, float height = 6.0f, float k = 3.0f)
{
    float d = 1e6;
    for (int i = 0; i < centers.size(); i++)
    {
...
public:
    ...
    double radius = 8.;
    float smoothK = 3.0f;
    zVector sunDir = zVector(1, 0.25, 0);

    ScalarField2D generatedField;
    int epoch = 0;

    std::vector<bool> lossOnBools = { true, false , true };
    ...
    )
    {
        ...
        for (int i = 0; i < N; i++)
        {
           ...
           // printf("%0.2f,%0.2f \n", angleErr, angleErr);
            ...
            //// Robust upstream search by ray marching
            ...
            //        std::vector<zVector> c = { centers[t] };
            //        std::vector<float> a = { angles[t] };
            //        float dist = blendOrientedBoxSDFs(probe, c, a);
            ...
            //            foundHit = true;
            //            break; // Stop stepping once we hit something
            //        }
            //    }
            ...
            if (t_exists)
            {
                ...
                for (int t = 0; t < number_sdf; t++)
                {
                    ...
                    bool hit = rayIntersectsFootprint(
                        rayOrigin,
                        rayDir,
                        centers[t],
                        angles[t],
                    ...
                    if (hit && hit_t >= 0 && hit_t <= maxDist)
                    {
                        ...
                        probe = hitPoint;
                        // Optional: you can store hitPoint if needed
            ...
            lossesByType[2][i] = (std::isnan(shadowOverlap)) ? 0 : shadowOverlap * shadowOverlap;
      ...
      //  generatedField.addVoronoi(trainingSamples);
        ...
        for (int i = 0; i < generatedField.RES; i++)
        {
            for (int j = 0; j < generatedField.RES; j++)
            {
                ...
                float d_p = polygonSDF(pt, polygon) + 5;


                generatedField.field[i][j] = d_c ;;/// min(d_c, -d_p); // min(d_v, -d_p);// min(min(-d_v, d_c), -d_p);
            }
        }
    ...
    void visualiseGradients( vector<float> &x)
    {
        ...
        for (int i = 0; i < number_sdf; i++)
        {
            glColor3f(0.5, 0.0, 1.0); // purple towers
            drawLine(zVecToAliceVec(centers[i]), zVecToAliceVec(centers[i] + zVector(0, 0, heights[i])*1));
        ...
        {
            glColor3f(0.5, 0.0, 1.0); // purple towers
            drawLine(zVecToAliceVec(centers[i]), zVecToAliceVec(centers[i] + zVector(0, 0, heights[i])));
...
void setup()
{
    ...
    loadPolygonFromCSV("data/polygon__.txt", polygon);
...
void draw()
{
   ...
   if(run) mlp.visualiseGradients(mlp_input_data);

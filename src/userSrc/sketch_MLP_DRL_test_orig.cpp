...
public:
    ...
    void decodeOutput(const std::vector<float>& out, std::vector<zVector>& centers, std::vector<float>& angles)
    {
        ...
        {
            int idx = i * 4;
            centers[i] = zVector(out[idx + 0], out[idx + 1], 0);
    ...
    float evaluateLoss(std::vector<zVector>& centers, std::vector<float>& angles)
    {
        ...
        std::vector<bool> normalizeLoss = { false, false }; // match number of loss types
